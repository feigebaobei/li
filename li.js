...
(function($) {
	...
	// 使用ajax方法加载html
	// 这种方法时异步加载方法的方法。需要使用后台服务器，只有前端页面时不能正常工作。
	$.fn.ajaxLoadHtml = function ( opts ) {
		var options = $.extend( {}, $.fn.ajaxLoadHtml.defaults, opts );
		return this.each( function () {
			var element = $(this);
			// console.log( element );
			// var url = options.url;
			if ( !options.url ) {
				console.log("请输入要加载的html文件的位置");
				// continue;
			}
			element.load( options.url, options.uploadData,options.secFu );
		});
	};
	$.fn.ajaxLoadHtml.defaults = {
		url: null,//指定需要加载的Html文件的位置
		uploadData: null,//向后台传输的数据
		secFu: function () {//加载成功后执行的代码
			console.log("加载成功！");
		}
	};
